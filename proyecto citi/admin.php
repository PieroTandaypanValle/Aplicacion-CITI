...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['actualizar'])) {
    ...
    // Validaciones
    if (!validarRut($rut_persona) || strlen($rut_persona) !== 9) {
        echo "<script>alert('El RUT debe tener 9 dígitos y ser válido.'); window.location.href='admin.php';</script>";
        exit;
    }
    if (!is_numeric($telefono) || strlen($telefono) !== 9) {
        echo "<script>alert('El teléfono debe tener 9 dígitos y ser numérico.'); window.location.href='admin.php';</script>";
        exit;
    }
    if (strtotime($fecha_nacimiento) > strtotime('2005-12-31')) {
        echo "<script>alert('La fecha de nacimiento no puede ser posterior a 2005.'); window.location.href='admin.php';</script>";
        exit;
    }
...
function validarRut($rut) {
    ...
    if ($digitoVerificador === 11) {
        ...
        $dv_calculado = $digitoVerificador;
    }
